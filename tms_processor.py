...
class ModernTMSProcessorGUI:
    def __init__(self, root):
        ...
        # Initialize detailed processor at startup
        try:
            from tms_detailed_processor import TMSDetailedDataProcessor
            self.detailed_processor = TMSDetailedDataProcessor()
            print("Detailed processor initialized successfully")
        except Exception as e:
            print(f"Warning: Could not initialize detailed processor: {e}")
        ...
        # Legacy savings history (for compatibility)
        self.savings_history_file = Path("savings_history.json")
        self.savings_history = self.load_savings_history()

        # Results history tracking (enhanced hybrid system)
        self.results_history_file = Path("logs") / "processing_results.json"
        self.results_history_file.parent.mkdir(exist_ok=True)
        self.results_history = self.load_results_history()
        self.compact_view = True  # Always compact view
    ...
    def create_widgets(self):
        ...
        self.create_enhanced_results_display()

        # Initialize enhanced results display
        self.update_enhanced_results_display()
    ...
    def browse_file(self):
        ...
        if file_paths:
            ...
            # No success message needed - files show in display
        else:
    ...
    def _process_file_thread(self):
        ...
        try:
            ...
            if report_type == "basic":
                ...
                # Use detailed processor (Cast Nylon processing)
                if self.detailed_processor is None:
                    self.root.after(0, lambda: messagebox.showerror("Error",
                        "Detailed processor not available. Please restart the application."))
                    self.root.after(0, self._reset_ui)
                    return
            ...
            if processed_count > 0:
                ...
                actual_report_type = self.report_type.get()  # Use the actual page type
                for i, stat in enumerate(all_stats):
                    ...
                    self.save_savings_history(file_stats, actual_report_type, 1, file_names=[file_name],
    ...
    def save_savings_history(self, stats, report_type, file_count, file_names=None, file_index=None, total_files=None):
        ...
        try:
            ...
            # Also add to new enhanced results system
            if file_names:
                filename = file_names[0] if len(file_names) == 1 else f"{len(file_names)} files"
            else:
            ...
            if display_type.lower() == 'basic':
                display_type = 'Basic'
            elif display_type.lower() == 'detailed':
                display_type = 'Cast Nylon'
            elif display_type.lower() == 'utc_main':
                display_type = 'UTC Main'
            elif display_type.lower() == 'utc_fs':
                display_type = 'UTC FS'
            ...
            self.add_result_entry(
                ...
                filename=filename,
                total_savings=float(stats.get('total_potential_savings', 0)),
                processed_rows=int(stats.get('total_loads', 0)),
    ...
    def create_enhanced_results_display(self):
        """Create enhanced results display with color-coded compact format"""
        # Main container for results display
        results_frame = tk.Frame(self.stats_outer_frame, bg=UI_COLORS['BACKGROUND_WHITE'])
        results_frame.pack(fill='both', expand=True, padx=2, pady=2)

        # Header with title and toggle button
        header_frame = tk.Frame(results_frame, bg=UI_COLORS['BACKGROUND_WHITE'])
        ...
        # No need for compact label anymore
        ...
        results_container = tk.Frame(results_frame, bg=UI_COLORS['BACKGROUND_WHITE'])
        results_container.pack(fill='both', expand=True)

        # Create text widget for results with scrollbar
        self.results_text = tk.Text(
            results_container,
            font=('Consolas', 9),  # Monospace font for aligned columns
            bg=UI_COLORS['BACKGROUND_WHITE'],
            fg=UI_COLORS['TEXT_PRIMARY'],
            wrap='word',
            relief='flat',
        ...
        self.results_text.tag_configure('date', foreground='#6b7280')  # Gray for dates
        self.results_text.tag_configure('processor', foreground='#3b82f6')  # Blue for processor type
        self.results_text.tag_configure('filename', foreground='#4b5563')  # Dark gray for filenames
        self.results_text.tag_configure('savings', foreground='#10b981')  # Green for savings amounts
        self.results_text.tag_configure('separator', foreground='#d1d5db')  # Light gray for separators

        # Scrollbar for results
        results_scrollbar = tk.Scrollbar(results_container, orient="vertical", command=self.results_text.yview)
    ...
    def load_results_history(self):
        """Load processing results history from JSON file"""
        try:
            if self.results_history_file.exists():
    ...
    def save_results_history(self):
        """Save processing results history to JSON file"""
        try:
    ...
    def add_result_entry(self, processor_type, filename, total_savings, processed_rows=None, file_index=None, total_files=None):
        """Add a new result entry to the history"""
        now = datetime.now()
        entry = {
        ...
        self.results_history = self.results_history[:50]  # Keep only last 50 entries
        self.save_results_history()
        self.update_enhanced_results_display()

    def update_enhanced_results_display(self):
        """Update the enhanced results display (compact view only)"""
        ...
        # Always show header (persistent)
        header = "DATE  TIME  | TYPE      | FILES | FILENAME           | SAVINGS\n"
        self.results_text.insert(tk.END, header, 'separator')
        self.results_text.insert(tk.END, "─" * 72 + "\n", 'separator')

        if not self.results_history:
            # Show placeholder when no data
            placeholder = "             Process files to see results here...\n"
            self.results_text.insert(tk.END, placeholder, 'date')
        else:
            # Show results in LIFO order (oldest first, newest at bottom)
            for entry in reversed(self.results_history):
                timestamp = datetime.fromisoformat(entry['timestamp'])
                date_str = timestamp.strftime('%m/%d %H:%M')

                # Use processor type as stored
                proc_type = entry['processor_type']

                # File count info
                if entry.get('file_index') and entry.get('total_files') and entry['total_files'] > 1:
                    file_info = f"{entry['file_index']}/{entry['total_files']}"
                else:
                    file_info = "1/1"
                ...
                # Format savings amount with cents
                savings = f"${entry['total_savings']:,.2f}"

                # Add with color tags and proper spacing
                self.results_text.insert(tk.END, date_str, 'date')
                self.results_text.insert(tk.END, " | ", 'separator')
                self.results_text.insert(tk.END, f"{proc_type:<9}", 'processor')
                self.results_text.insert(tk.END, " | ", 'separator')
                self.results_text.insert(tk.END, f"{file_info:<5}", 'date')
                self.results_text.insert(tk.END, " | ", 'separator')
                self.results_text.insert(tk.END, f"{filename:<18}", 'filename')
                self.results_text.insert(tk.END, " | ", 'separator')
                self.results_text.insert(tk.END, savings, 'savings')
    ...
    def clear_results_history(self):
        """Clear all results history with confirmation"""
        if not self.results_history:
            return

        result = messagebox.askyesno(
            "Clear Results History",
            f"Are you sure you want to clear all {len(self.results_history)} processing results?\n\n"
            "This action cannot be undone.",
            icon='warning'
        )
        ...
        if result:
            ...
            try:
                if self.results_history_file.exists():
                    self.results_history_file.unlink()
            except Exception as e:
                print(f"Warning: Could not delete results history file: {e}")

            self.update_enhanced_results_display()
            messagebox.showinfo("Cleared", "Processing results history has been cleared.")
    ...
    def process_marmon_file(self):
        ...
        try:
            ...
            if output_file:
                ...
                # Calculate potential savings from the processed data
                total_savings = 0
                if 'Potential Savings' in processed_df.columns:
                    total_savings = processed_df['Potential Savings'].sum()

                # Add to enhanced results system
                self.add_result_entry(
                    processor_type=f"{selected_client} Marmon",
