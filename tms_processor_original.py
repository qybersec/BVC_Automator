...
class UTCFSProcessor:
    """
    UTC FS processor using Basic_Processor logic
    with Destination City sorting and Green River rule
    """

    def __init__(self):
        # File structure settings (same as UTC Main format)
        self.HEADER_ROW = 8          # Row 9 in Excel (0-indexed) - actual header row
        # Row 11 in Excel (0-indexed) - first data row (skip row 9 which is empty)
    ...
    def process_excel_file(self, file_path):
        ...
        # Step 4: Sort by destination city (same as UTC Main)
        if 'Destination City' in df.columns:
            df = df.sort_values('Destination City', na_position='last')
            print(f"Sorted by Destination City - {len(df)} rows")
    ...
    def load_data(self, file_path):
        ...
        # Map UTC FS specific column names to expected format (same as UTC Main)
        data_df = self._map_utc_columns(data_df)
    ...
    def apply_business_rules(self, df):
        """Apply TMS business rules using Basic_Processor logic"""
        df = df.copy()
        print("Applying UTC FS business rules...")
        ...
        # Rule 6: Green River City Rule (UTC FS specific)
    ...
    def _apply_green_river_rule(self, df):
        """
        Rule 6: Green River City Rule - Zero out PS for any
        Green River origin or destination
        """
        green_river_count = 0
        ...
        if green_river_count > 0 and 'Potential Savings' in df.columns:
            ...
            print(f"Green River City Rule: Zeroed out PS for {green_river_count} rows")
        elif green_river_count > 0:
            print(
                f"Green River City Rule: Found {green_river_count} Green River rows "
                f"but no Potential Savings column"
            )
        else:
            print("Green River City Rule: No Green River cities found")
    ...
    def _map_utc_columns(self, df):
        """Map UTC FS column names to expected format (same logic as UTC Main)"""
        df = df.copy()
...
class TranscoProcessor:
    """Transco processor using Basic_Processor logic with Destination City sorting"""

    def __init__(self):
        # File structure settings (updated for Transco format)
        self.HEADER_ROW = 8          # Row 9 in Excel (0-indexed) - actual header row
        # Row 11 in Excel (0-indexed) - first data row (skip row 9 which is empty)
        self.DATA_START_ROW = 10
    ...
    def load_data(self, file_path):
        ...
        # Map Transco specific column names to expected format
        data_df = self._map_utc_columns(data_df)
    ...
    def apply_business_rules(self, df):
        """Apply TMS business rules using Basic_Processor logic"""
        df = df.copy()
        print("Applying Transco business rules...")
        ...
        # Rule 6: Miles City Rule (Transco specific)
    ...
    def _apply_miles_city_rule(self, df):
        """Rule 6: Miles City Rule - Zero out PS for any Miles City origin or destination"""
        miles_city_count = 0
    ...
    def _map_utc_columns(self, df):
        """Map Transco column names to expected format"""
        df = df.copy()
